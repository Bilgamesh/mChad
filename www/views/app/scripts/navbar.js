...
async function Navbar() {
  ...
  function updateSelection() {
    ...
    for (const button of document.getElementById('navbar-bottom').children) {
  ...
  function show() {
    document.getElementById('navbar-bottom').style.display = '';
  }
    ...
    for (const key in router.ROUTES)
      if (
        ...
        document
          ...
          .addEventListener('click', router.route);
      }
  }
  ...
  function displayBadge({ element, id, number }) {
    const existingBadge = document.getElementById('' + id);
    if (number === 0) {
